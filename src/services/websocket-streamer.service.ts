...
export default class WebsocketStreamerService {
    private static instance: WebsocketStreamerService;
    private server : WebsocketServer;
    private connections : connection[];
    private rtsp : any[];

    static getInstance(): WebsocketStreamerService | null {
        if (!WebsocketStreamerService.instance) {
            console.log("Websocket service not initialized.");
            return null;
        }
        return WebsocketStreamerService.instance;
    }

    static async initialize(server : http.Server): Promise<void> {
        if (!WebsocketStreamerService.instance) {
            WebsocketStreamerService.instance = new WebsocketStreamerService(server);
        }
    }
